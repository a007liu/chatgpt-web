...
import { NAutoComplete, NButton, NInput, NSelect, NSpace, NSpin, NDrawer, useDialog, useMessage } from 'naive-ui'
...
const footerClass = computed(() => {
	let classes = ['p-4']
	if (isMobile.value)
		classes = ['sticky', 'left-0', 'bottom-0', 'right-0', 'py-2', 'overflow-hidden']
	return classes
})
	...
	<div class="flex flex-col w-full h-full">
		...
		<footer :class="footerClass">
			<div class="w-full max-w-screen-xl m-auto">
				<NSpace vertical>
					<div class="flex items-center p-2 border-b border-t border-[#eee]">
						<HoverButton @click="handleClear">
              <span class="text-xl text-[#4f555e] dark:text-white">
                <SvgIcon class="text-xl text-[#666666]" icon="ri:delete-bin-line"/>
              </span>
						</HoverButton>
              ...
              <span class="text-xl text-[#4f555e] dark:text-white">
                <IconPrompt class="w-[20px] m-auto"/>
						...
						<HoverButton
							...
							:class="{ 'text-[#5D96FD]': usingContext, 'text-[#a8071a]': !usingContext }"
							@click="handleToggleUsingContext"
						>
              <span class="text-xl">
                <SvgIcon icon="ri:chat-history-line"/>
              </span>
							<span>{{ usingContext ? '包含上下文' : '不含上下文' }}</span>
						</HoverButton>
						<NSelect
							class="flex-1 ml-2 max-w-[300px]"
							:value="currentChatModel"
							:options="authStore.session?.chatModels"
							:disabled="!!authStore.session?.auth && !authStore.token"
							@update-value="(val) => handleSyncChatModel(val)"
						/>
						<SvgIcon v-if="isMobile"
										 class="text-xl text-[#4f555e] ml-1"
										 icon="lucide:circle-ellipsis"
										 @click="handleShowMore"/>
					</div>
					<div class="flex items-center justify-between p-2 space-x-2">
						<NAutoComplete
							class="input-no-border dialog-input"
						...
						<NButton circle type="primary" :disabled="buttonDisabled" @click="handleSubmit">
							<template #icon>
                <span class="dark:text-black rounded">
		...
		<!--	保存未图片、提词器	-->
		<NDrawer v-model:show="isMore" placement="bottom" :close-on-esc="false" :mask-closable="false">
			<div>
				<div class="flex py-12">
					<div class="flex-1 flex flex-col items-center justify-center">
						<SvgIcon
							class="flex-1 mb-1 text-7xl text-[#5D96FD]"
							icon="iconoir:download-circle-solid"
							@click="handleExport"/>
						<span class="text-[#000] font-bold">保存为图片</span>
					</div>
					<div class="flex-1 flex flex-col items-center justify-center">
						<SvgIcon
							class="flex-1 mb-1 text-7xl text-[#5D96FD]"
							icon="ant-design:right-circle-filled"
							@click="showPrompt = true"/>
						<span class="text-[#000] font-bold">提示词</span>
					</div>
				</div>
				<div class="px-4">
					<NButton type="primary" size="large" color="#E9F0FD" text-color="#5D96FD" block round @click="isMore=false">
						取消
					</NButton>
				</div>
			</div>
