...
export const useChatStore = defineStore('chat-store', {
  ...
  actions: {
    async syncHistory(callback: () => void) {
      ...
      if (uuid == null) {
        await this.addHistory({ title: '新对话', uuid: Date.now(), isEdit: false, usingContext: true })
      }
      else {
        this.active = uuid
    ...
    async syncChat(h: Chat.History, lastId?: number, callback?: () => void,
      ...
      const history = this.history.filter(item => item.uuid === h.uuid)[0]
      if (history === undefined || history.loading || history.all) {
        if (lastId === undefined) {
          // 加载更多不回调 避免加载概率消失
          callback && callback()
        }
        if (history?.all ?? false)
    ...
    async clearLocalChat() {
      ...
      else await router.replace({ path: '/login' })
    },
